...
export default class CheckPickSession extends Session {
  ...
  async amend(commitInfo) {
    ...
    if (commitInfo.shas.length === 1) {
      ...
      const canFinal = await this._amend();
      if (!canFinal) {
        return;
      }
  ...
  async apply() {
    ...
    await this.amend(commitInfo);
  }

  async _amend() {
    const { cli } = this;
    if (!this.readyToAmend()) {
      cli.warn('Not yet ready to amend, run `git node cherry-pick --abort`');
      return;
    }
    this.startAmending();
