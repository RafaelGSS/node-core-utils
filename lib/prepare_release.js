...
export default class ReleasePreparation {
  ...
  async cherryPickSecurityPRs(filterLabels) {
    ...
    for (const pr of prs) {
      ...
      if (amend) {
        const cp = new CherryPick(pr.number, this.owner, this.repo, this.dir, cli);
        const success = await cp.start(metadata);
        if (!success) {
          const partialSuccessMsg = atLeastOneValidPR
            ? 'One or more PRs were cherry-picked with success.'
            : '';
