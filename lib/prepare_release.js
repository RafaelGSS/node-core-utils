...
import LandingSession from './landing_session.js';
...
export default class ReleasePreparation {
  ...
  async cherryPickSecurityPRs(filterLabels) {
    ...
    if (prs.length === 0) {
      cli.warn('There are no PRs available in nodejs-private');
      return false;
    }

    let atLeastOneValidPR = false;
    for (const pr of prs) {
      if (pr.mergeable !== 'MERGEABLE') {
        this.warnForInvalidPR(pr);
        continue;
      }
      atLeastOneValidPR = true;
      const metadata = await getMetadata({
        prid: pr.number,
        owner: 'nodejs-private',
        repo: 'node-private'
      }, false, cli);
      // TODO: check missing CVE-ID metadata
      const amend = await cli.prompt(
        'Would you like to amend this PR to the proposal?',
        { default: true }
      );
      if (amend) {
        const land = new LandingSession(cli, request, process.cwd());
        await land.start(metadata);
      }
    }
    return atLeastOneValidPR;
  }
